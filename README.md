...
3. **工作流文件名规范（核心）**  
   ...
   - **格式**：**`工作流名+文本N+图片M+视频K.json`**  
     ...
     - `文本N`：工作流内最多可由用户传入 N 段文本；少传或不传时保留工作流中 Simple String 节点的默认值。  
     - `图片M`：工作流内须有 M 个 ETN_LoadImageBase64 节点，对应 M 张图；图片数量仍是强限制。  
     - `视频K`：工作流内须有 K 个 VHS_LoadVideo 节点；视频数量仍是强限制，若无视频可省略整段「+视频K」。  
   - **带输出文本的变体**：**`工作流名+图片1=文本1.json`** 等，用于表示最多展示 ComfyUI 输出的 N 条文本。只有带 `=文本N` 的工作流才会外显 history 中的文本输出。
...
- `文本N` 表示最多传入 N 段文本；少传时保留工作流默认值。`图片M` / `视频K` 仍表示必须提供的图片/视频数量。  
- 需要展示 ComfyUI 文本输出时，请使用 **`=文本N`** 文件名后缀，例如 `反推+图片1=文本1.json`。  
